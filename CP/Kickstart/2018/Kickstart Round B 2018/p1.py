...
def numbertolist(number):
    a=[int(i) for i in list(str(number))]
    return a
...
for __ in range(readInt()):
    ...
    l2 = len(b)
    mini = min(l1,l2)-1
    sumi = abs(int(a)-int(b))+1
    # print int(a[:l1-mini]),int(b[:l2-mini])
    count = 0
    if int(b[-1:])>=9:
        count =1
    # print int(a[-1:]),int(b[-1:])
    if int(a[:l1-mini])<9*pow(10,mini-1)<int(b[:l2-mini]):
        print "loop1"
        ...
        ra = int(a)%9
        print 9*pow(10,mini),sumi
        niniec = ((9*pow(10,mini)-1-int(a)+(9-ra))/9)+(int(b)-(9*pow(10,mini)+(9-ra))/9)+1
        print sumi,niniec
        sumi-=niniec
    else:
        print "loop2"
        if mini-1>=0:
            sumi-=pow(10,mini-1)
        ra = int(a)%9
        niniec = ((int(b)-int(a)+(9-ra))/9)+count
        print sumi,niniec
        sumi-=niniec
    print sumi

    # print "Case #{}: {}".format(__+1,sumi)
