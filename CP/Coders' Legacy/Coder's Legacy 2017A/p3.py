...
def p():
    art=[readStr() for i in xrange(input())]
    # print art
    arr=[[ord(item[0]),ord(item[-1])] for item in art]
    # print arr
    turns=0
    n=len(arr)
    # loda = 0
    for it in permutations(arr,n):
        temp=j=1
        # loda+=1
        # print it,loda
        while j<n and it[j-1][-1]==it[j][0]:
            temp+=1
            j+=1
        turns=max(turns,temp)
    return turns
print p()
