...
def readnumbertolist(stra):
    a=[int(i) for i in list(stra)]
    return a
...
for __ in range(readInt()):
    ar = ["red","blue","pink","white","black","violet","cyan","yellow","green","brown"]
    n = readInt()
    arr = readInts()
    maxi = -1e9
    for i in range(len(arr)):
        a = readnumbertolist(str(arr[i]))
        m = len(a)
        for j in range(m):
            sumi = sum(a)
            a = readnumbertolist(str(sumi))
            # print a
            if len(a)==1:
                # print a[0]
                if maxi < int(a[0]):
